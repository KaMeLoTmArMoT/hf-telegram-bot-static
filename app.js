...
function renderJobQueue() {
    ...
    [...trackedJobs].reverse().forEach((job) => {
        ...
        const currentStageIdx =
            ...
            job.status === 'transcribing_groq' ? 2 :
            isUploading ? 1 : 1;
        ...
        card.innerHTML = `
            <div class="job-top-row">
                ...
                <span class="job-status-pill">${isCompleted ? '✓ Ready' : isFailed ? '✕ Error' : isUploading ? '🚀 Uploading' : (job.progress || 10) + '%'}</span>
            </div>
            <div class="job-stage-desc">${job.stage_label || 'Processing...'}</div>
            ${!isCompleted && !isFailed ? `
                <div class="job-progress-bg">
                    <div class="job-progress-fill" style="width: ${job.progress || 15}%"></div>
                </div>
            ` : ''}
            ${stepsHtml}
            ${isCompleted ? '<div class="job-view-btn">📄 Open Transcript →</div>' : ''}
        `;
